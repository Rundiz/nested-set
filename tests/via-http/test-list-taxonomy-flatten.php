...
if (isset($_GET['search']) && trim($_GET['search']) != null) {
    ...
    $options['search']['search_value'] = trim($_GET['search']);
}
    ...
    <body>
                ...
                <select name="filter_parent_id">
                    ...
                    if (isset($list_all_txn['items']) && is_array($list_all_txn['items'])) {
                        foreach ($list_all_txn['items'] as $row) {
                            echo '<option';
                            echo ' value="' . $row->{$NestedSet->id_column_name} . '"';
                            if (isset($_GET['filter_parent_id']) && $_GET['filter_parent_id'] == $row->{$NestedSet->id_column_name}) {
                                echo ' selected=""';
                            }
                            echo '>';
                            if ($row->{$NestedSet->level_column_name} > 1) {
                                echo str_repeat(' &nbsp; &nbsp;', ($row->{$NestedSet->level_column_name} - 1));
                                echo '|&mdash;';
                            }
                            echo $row->name;
                            echo '</option>' . "\n";
        ...
        <table class="list-taxonomy-table">
            ...
            <tbody>
                ...
                ?> 
                <tr>
                    <td><?php echo $row->{$NestedSet->id_column_name}; ?></td>
                    <td><?php echo $row->{$NestedSet->parent_id_column_name}; ?></td>
                    ...
                    <td><?php echo $row->{$NestedSet->position_column_name}; ?></td>
                    <td><?php echo $row->{$NestedSet->level_column_name}; ?></td>
                    <td><?php echo $row->{$NestedSet->left_column_name}; ?></td>
                    <td><?php echo $row->{$NestedSet->right_column_name}; ?></td>
                </tr>
                <?php 
                    }// endforeach;
                    unset($row);
