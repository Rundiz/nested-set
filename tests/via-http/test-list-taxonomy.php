...
function renderTaxonomyTree(array $array, \Rundiz\NestedSet\NestedSet $NestedSet = null, $first = true)
{
    ...
    foreach ($array as $item) {
        ...
        $output .= 'id: ' . $item->{$NestedSet->idColumnName} . ', ';
        $output .= 'parent_id: ' . $item->{$NestedSet->parentIdColumnName} . ', ';
        $output .= 'position: ' . $item->{$NestedSet->positionColumnName} . ', ';
        $output .= 'level: ' . $item->{$NestedSet->levelColumnName} . ', ';
        $output .= 'left: ' . $item->{$NestedSet->leftColumnName} . ', ';
        $output .= 'right: ' . $item->{$NestedSet->rightColumnName};
        $output .= '</span>';
        $output .= '</div>' . "\n";
